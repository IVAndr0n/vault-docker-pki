...
listener "tcp" {
  ...
  tls_disable           = 0
  tls_cert_file         = "${docker_vault_dir_tls}/${ca_cert_file_name_vault_server}.pem"
  tls_key_file          = "${docker_vault_dir_tls}/${ca_key_file_name_vault_server}.key"
  tls_client_ca_file    = "${docker_vault_dir_crt}/${ca_cert_file_name_intermediate}.pem"
}
storage "file" {
...
sudo chown -R ${USER}:$(id -gn ${USER}) ${vault_dir_crt}

sudo chmod -R 0555 ${vault_dir_tls}
sudo chown -R ${USER}:$(id -gn ${USER}) ${vault_dir_tls}

sudo chmod -R 0555 ${vault_dir_config}
sudo chown -R ${USER}:$(id -gn ${USER}) ${vault_dir_config}

sudo chmod 0555 ${vault_dir_config}/${vault_file_config}
